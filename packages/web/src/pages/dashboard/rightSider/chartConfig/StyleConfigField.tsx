...
import { COMPONENT_NAME, IChartConfig, isWidgetComponentName } from '@block-bi/material'
...
const StyleConfigField = ({ cardId, configKey }: StyleConfigFieldProps) => {
  const config = useDashboardStore((state) => {
    ...
    if (card?.componentName !== 'chart' && !isWidgetComponentName(card?.componentName)) {
      return undefined
    }
    return card.props.styleConfig.find((item) => item.key === configKey)
  })
