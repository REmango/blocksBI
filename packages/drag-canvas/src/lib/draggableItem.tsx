...
const DraggableItem: React.FC<DraggableItemProps> = (props) => {
  ...
  // Sync external layout (e.g. agent patchPageLayout) into internal drag state.
  useEffect(() => {
    if (!layout) return
    if (context.active?.id === id) return
    if (isResizing.current) return

    const nextX = lockX ?? layout.x
    ...
    transformRef.current = null
  }, [layout?.x, layout?.y, layout?.width, layout?.height, lockX, id, context.active?.id])
